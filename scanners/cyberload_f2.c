...
void cyberload_f2_search(void)
{
   ...
   unsigned char decryptor[7]= {0x59,0x14,0x03,0x88,0x10,0xFA,0x9D};

   /* loader 2: deciphering data. (as found at $0314) */
   unsigned char decryptorcodes[16] = {0xA6,0x02,0x6B,0x8E,0xC1,0xFE,0xBC,0x8D,
                             0x8B,0x49,0x7F,0x8D,0x60,0xD1,0x8D,0xE1};

   /* loader 2/3/4: set threshold pattern. */
   unsigned char set_thres[10]={0xA9,XX,0x8D,0x04,0xDC,0xA9,XX,0x8D,0x05,0xDC};

   /* loader 2/3/4: set pilot/sync pattern. */
   unsigned char set_pilot[7]= {0xC9,XX,0xF0,XX,0xC9,XX,0xD0};
   ...
   {
      /* make a copy of the data (need a larger buffer to operate in)... */
      bufsz= blk[t]->cx;

      /* The buffer allocated below is too small (causes buffer overrun(s))
         without the +200 (on ie. Last Ninja), why???...
      ...
      buf= (unsigned char*)malloc((bufsz+200));
      for(i=0; i<bufsz; i++)
         buf[i]= blk[t]->dd[i];
