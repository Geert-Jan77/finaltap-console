...
void virgin_search(void)
{
   ...
   {
      if((z=find_pilot(i,VIRGIN))>0)
      {
         sof=i;
         i=z;
         if(readttbyte(i, ft[VIRGIN].lp, ft[VIRGIN].sp, ft[VIRGIN].tp, MSbF)==ft[VIRGIN].sv)
         {
            sod=i+8;

            /* decode the header, so we can validate the addresses */
            for(tcnt=0; tcnt<HDSZ; tcnt++)
               hd[tcnt]= readttbyte(sod+(tcnt*8), ft[VIRGIN].lp, ft[VIRGIN].sp, ft[VIRGIN].tp, ft[VIRGIN].en);

            s= hd[2]+ (hd[3]<<8);  /* get start address */
            e= hd[4]+ (hd[5]<<8);  /* get end address */
            ...
            if(e>s)
            {
               eod= sod+ ((x+HDSZ)*8);
               eof= eod+7;
               addblockdef(VIRGIN, sof,sod,eod,eof, 0);
...
int virgin_describe(int row)
{
   ...
   {
      b= readttbyte(s+(i*8), ft[VIRGIN].lp, ft[VIRGIN].sp, ft[VIRGIN].tp, ft[VIRGIN].en);
      hd[i]=b;
   }
